...
2. **切换分身**：
   ...
   > **注意**：
   >
   > - 在 **Terminal 交互模式**下，大圣会直接“变身”切换。
   > - 在 **Web 界面**下，请直接点击左侧分身列表进行切换，对话上下文会自动隔离。
   > - 在 **飞书** 等外部渠道，暂不支持通过对话切换分身，请使用 Web 界面管理或通过不同群聊接入（未来支持）。
...
2. **启动 Web 界面**：
   ```bash
   monkeyking server --web
   ```
   *(启动后在浏览器打开 http://127.0.0.1:8000 即可使用 Web 界面与大圣及各分身进行交互。)*
